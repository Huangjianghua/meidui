...
public class BasicOpV1Controller {
	...
	ResBodyData exit(){
		...
		try {
			RedisTemplate.getJedisInstance().execDelToCache(reqJson.getString(ConstSysParamsDefination.TOKEN));
		} catch (Exception e) {
	...
	ResBodyData getMemIdByToken(){
		...
		String memId=reqJson.getString(ConstSysParamsDefination.MEMID);
		Map<String, Object> data=new HashMap<>();
		data.put(ConstSysParamsDefination.MEMID,memId);
	...
	ResBodyData checkToken(){
		...
		if(reqJson.containsKey(ConstSysParamsDefination.MEMID)){
