...
<body>
<form action="${ctx}/searchLog/listSearchLog.do"  method="post">
       ...
       <tbody id="tbody">
          <c:forEach items="${pageView.dateList}" var="item" varStatus="temp">
	        <tr class="${temp.index % 2 == 0 ?'':'even' }">
						<td>&nbsp;${item.keyword}</td>
						<td><div style="width:300px;line-height:22px;margin:0 auto;word-wrap:break-word;">&nbsp;${item.ip}</div></td>
						<td>&nbsp;${item.resultNum}</td>
						<td>&nbsp;${item.type}</td>
						<td>&nbsp;${item.searchTime}</td>
						<td>
						  <a href="${ctx}/searchLog/deleteSearchLog.do?logId=${item.logId}">删除</a>
						</td>
		      </tr>
          </c:forEach>
          </tbody>
