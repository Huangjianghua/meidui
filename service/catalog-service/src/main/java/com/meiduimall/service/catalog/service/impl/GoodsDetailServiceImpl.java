...
public class GoodsDetailServiceImpl implements GoodsDetailService {
	...
	public ResBodyData getItemDetailById(String mem_id, Integer item_id) {
		...
		itemData.setSales_volume(String.valueOf(rate_count));
		...
		itemData.setItme_id(String.valueOf(item_id));
		...
		if (store != null) {
			if (freez != null) {
				...
				itemData.setItem_store(String.valueOf(item_store));
			} else {
		...
		if (itemSkuWithBLOBsList != null && itemSkuWithBLOBsList.size() > 0) {
			...
			for (int i = 0; i < itemSkuWithBLOBsList.size(); i++) {
				...
				result_sku.setSku_store(String.valueOf(sku_store));
