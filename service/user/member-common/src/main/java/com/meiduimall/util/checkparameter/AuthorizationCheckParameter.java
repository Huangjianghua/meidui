package com.meiduimall.util.checkparameter;
	...
	private JSONObject vilidateCommonParameter(JSONObject obj,String[] args){
		...
		if (obj.containsKey("user_id")) {
			...
			if (StringUtil.isEmptyByString(tmpObj)
				...
				result.put(SysParaNameConst.STATUS_CODE, "1001");
				result.put(SysParaNameConst.RESULT_MSG, "用户名输入错误");
				return result;
			}
		...
		if (obj.containsKey("pass_word")) {
			...
			if (password.length() != 32 && password.length() != 65) {
				result.put("boolean_result", false);
				result.put(SysParaNameConst.STATUS_CODE, "1009");
				result.put(SysParaNameConst.RESULT_MSG, "密码输入错误");
				return result;
			}
		}
		...
		if (obj.containsKey("phone")) {
			...
			if (StringUtil.isEmptyByString(phone)) {
				result.put("boolean_result", false);
				result.put(SysParaNameConst.STATUS_CODE, "1021");
				result.put(SysParaNameConst.RESULT_MSG, "手机号错误");
				return result;
			}
		...
		if (obj.containsKey("email")) {
			...
			if (StringUtil.isEmptyByString(email)
					|| email.length() > 100) {
				result.put("boolean_result", false);
				result.put(SysParaNameConst.STATUS_CODE, "1023");
				result.put(SysParaNameConst.RESULT_MSG, "邮箱地址错误");
				return result;
			}
	...
	private JSONObject vilidatePageParameter(JSONObject obj){
		...
		if(obj.containsKey("current_page")){
			...
			if (StringUtil.isEmptyByString(tmpObj)
				...
				result.put("boolean_result", false);
				result.put(SysParaNameConst.STATUS_CODE, 9994);
				result.put(SysParaNameConst.RESULT_MSG, "参数数值类型输入错误 [current_page=" + tmpObj + "]");
				return result;
			}
		}
		...
		if(obj.containsKey("page_size")){
			...
			if (StringUtil.isEmptyByString(tmpObj)
				...
				result.put("boolean_result", false);
				result.put(SysParaNameConst.STATUS_CODE, 9994);
				result.put(SysParaNameConst.RESULT_MSG, "参数数值类型输入错误 [page_size=" + tmpObj + "]");
				return result;
			}
		}
	...
	public JSONObject vilidateMemberParameter(JSONObject obj,String[] args){
		...
		if(obj.containsKey("is_default")){
			...
			if (StringUtil.isEmptyByString(tmpObj)) {
				result.put("boolean_result", false);
				result.put(SysParaNameConst.STATUS_CODE, 9994);
				result.put(SysParaNameConst.RESULT_MSG, "参数数据类型输入错误 [is_default=" + tmpObj + "]");
				return result;
			}else if(!(ApplicationConstant.BANK_IS_DEFAULT_YES.equals(tmpObj)
					|| ApplicationConstant.BANK_IS_DEFAULT_NO.equals(tmpObj))){
				result.put("boolean_result", false);
				result.put(SysParaNameConst.STATUS_CODE, "2024");
				result.put(SysParaNameConst.RESULT_MSG, "默认银行卡类型输入错误");
				return result;
			}
		}
		...
		if(obj.containsKey("change_type")){
			...
			if (StringUtil.isEmptyByString(tmpObj)) {
				result.put("boolean_result", false);
				result.put(SysParaNameConst.STATUS_CODE, 9994);
				result.put(SysParaNameConst.RESULT_MSG, "参数数据类型输入错误 [change_type=" + tmpObj + "]");
				return result;
			}else if(!(ApplicationConstant.CHANGE_TYPE_UPDATE.equals(tmpObj)
					|| ApplicationConstant.CHANGE_TYPE_DELETE.equals(tmpObj))){
				result.put("boolean_result", false);
				result.put(SysParaNameConst.STATUS_CODE, "2025");
				result.put(SysParaNameConst.RESULT_MSG, "变更类型输入错误");
				return result;
			}
		}
	...
	public JSONObject vilidateOrderParameter(JSONObject obj,String[] args){
		...
		if (obj.containsKey("order_source")) {
			...
			if (StringUtil.isEmptyByString(order_source)) {
				result.put("boolean_result", false);
				result.put(SysParaNameConst.STATUS_CODE, "2017");
				result.put(SysParaNameConst.RESULT_MSG, "订单来源输入错误");
				return result;
			}
		...
		if (obj.containsKey("order_type")) {
			...
			if (StringUtil.isEmptyByString(tmpObj)) {
				result.put("boolean_result", false);
				result.put(SysParaNameConst.STATUS_CODE, "2017");
				result.put(SysParaNameConst.RESULT_MSG, "订单类型输入错误");
				return result;
			}
		}
		...
		if(obj.containsKey("apply_carry_cash")){
			...
			if (StringUtil.isEmptyByString(tmpObj)
				...
				result.put("boolean_result", false);
				result.put(SysParaNameConst.STATUS_CODE, "2016");
				result.put(SysParaNameConst.RESULT_MSG, "提现余额输入错误");
				return result;
			}
		}
		...
		if(args == null || args.length == 0){
			...
			for (String key : args) {
				if(!obj.containsKey(key)){
					result.put("boolean_result", false);
					result.put(SysParaNameConst.STATUS_CODE, 9998);
					result.put(SysParaNameConst.RESULT_MSG, "缺少请求参数 [" + key + "]");
					break;
				}
				if(obj.get(key) == null){
					result.put("boolean_result", false);
					result.put(SysParaNameConst.STATUS_CODE, 9998);
					result.put(SysParaNameConst.RESULT_MSG, "请求参数 [" + key + "] 不能为空");
					break;
				}
			}
		}
	...
	public static JSONObject vilidetePar(JSONObject obj) {
		...
		if (obj.containsKey("rise_user_id")) {
			...
			if (StringUtil.isEmptyByString(rise_user_id)
				...
				result.put("boolean_result", false);
				result.put(SysParaNameConst.STATUS_CODE, "2061");
				result.put(SysParaNameConst.RESULT_MSG, "升级为商家会员的登录名输入错误");
				return result;
			}
		...
		if (obj.containsKey("sign_source")) {
			...
			if (!"O2O".equals(sign_source) && !"1gw".equals(sign_source)&&!"md1gw".equals(sign_source)) {
				result.put("boolean_result", false);
				result.put(SysParaNameConst.STATUS_CODE, "2039");
				result.put(SysParaNameConst.RESULT_MSG, "注册来源输入错误");
				return result;
			}
		}
		// 注册类型
		if (obj.containsKey("register_type")) {
			...
			if (!"2".equals(register_type) && !"1".equals(register_type)) {
				result.put("boolean_result", false);
				result.put(SysParaNameConst.STATUS_CODE, "2040");
				result.put(SysParaNameConst.RESULT_MSG, "注册类型输入错误");
				return result;
			}
		}
		if (obj.containsKey("order_id")) {
			...
			if (StringUtil.isEmptyByString(order_id)
					|| StringUtil.stringFilters(order_id).length() != order_id
							...
							|| order_id.length() > 40) {
				result.put("boolean_result", false);
				result.put(SysParaNameConst.STATUS_CODE, 2013);
				result.put(SysParaNameConst.RESULT_MSG, "用户产品的订单号输入错误");
				return result;
			}
		}
		if (obj.containsKey("consumer_money")) {
			...
			if (StringUtil.isEmptyByString(consumer_money)
				...
				result.put("boolean_result", false);
				result.put(SysParaNameConst.STATUS_CODE, 2016);
				result.put(SysParaNameConst.RESULT_MSG, "消费金额输入错误");
				return result;
			}
		}
		if (obj.containsKey("product_name")) {// 消费项目名称
			...
			if (StringUtil.isEmptyByString(product_name)
				...
				result.put("boolean_result", false);
				result.put(SysParaNameConst.STATUS_CODE, 2015);
				result.put(SysParaNameConst.RESULT_MSG, "消费项目名称输入错误");
				return result;
			}
		}
		if (obj.containsKey("back_integral")) {
			...
			if (StringUtil.isEmptyByString(back_integral)
				...
				result.put("boolean_result", false);
				result.put(SysParaNameConst.STATUS_CODE, 2014);
				result.put(SysParaNameConst.RESULT_MSG, "消费返还积分数量输入错误");
				return result;
		...
		if (obj.containsKey("orginal_user_id")) {
			...
			if (StringUtil.isEmptyByString(orginal_user_id)
					|| StringUtil.stringFilters(orginal_user_id).length() != orginal_user_id
							.length()) {
				result.put("boolean_result", false);
				result.put(SysParaNameConst.STATUS_CODE, 2018);
				result.put(SysParaNameConst.RESULT_MSG, "系统会员ID输入错误");
				return result;
			}
		}
		if (obj.containsKey("pay_type")) {
			...
			if (StringUtil.isEmptyByString(mch_pay_type)) {
				result.put("boolean_result", false);
				result.put(SysParaNameConst.STATUS_CODE, 2019);
				result.put(SysParaNameConst.RESULT_MSG, "支付类型输入错误");
				return result;
			}
		...
		if (obj.containsKey("consume_coupon_count")) {
			...
			if(!"0".equals(consume_coupon_count)){
				if (!StringUtil.checkFloat(consume_coupon_count, "0+")) {
					result.put("boolean_result", false);
					result.put(SysParaNameConst.STATUS_CODE, 2037);
					result.put(SysParaNameConst.RESULT_MSG, "消费劵金额输入错误");
					return result;
		...
		if (obj.containsKey("authorized_type")) {
			...
			if (!"1".equals(authorized_type)
				...
				result.put("boolean_result", false);
				result.put(SysParaNameConst.STATUS_CODE, 9997);
				result.put(SysParaNameConst.RESULT_MSG, "非法操作");
				return result;
		...
		if(obj.containsKey("mem_sex")){
			...
			if(!"男".equals(mem_sex)
				&& !"女".equals(mem_sex)	){
				result.put("boolean_result", false);
				result.put(SysParaNameConst.STATUS_CODE, 2002);
				result.put(SysParaNameConst.RESULT_MSG, "性别输入错误");
				return result;
			}
		...
		if(obj.containsKey("mem_pic")){//http://127.0.0.1/name_1123
			...
			if(StringUtil.isEmptyByString(mem_pic)){
				result.put("boolean_result", false);
				result.put(SysParaNameConst.STATUS_CODE, 2013);
				result.put(SysParaNameConst.RESULT_MSG, "头像输入错误");
				return result;
			}
		...
		if(obj.containsKey("mem_birthday")){
			...
			if(!StringUtil.checkDate(mem_birthday)){
				result.put("boolean_result", false);
				result.put(SysParaNameConst.STATUS_CODE, 2001);
				result.put(SysParaNameConst.RESULT_MSG, "会员生日输入错误");
				return result;
			}
		...
		if(obj.containsKey("mem_address_area")){
			...
			if(StringUtil.isEmptyByString(mem_address_area)
				...
				result.put("boolean_result", false);
				result.put(SysParaNameConst.STATUS_CODE, 2014);
				result.put(SysParaNameConst.RESULT_MSG, "会员所属省市县(区)输入错误");
				return result;
			}
		...
		if(obj.containsKey("mem_address")){
			...
			if(StringUtil.isEmptyByString(mem_address)
				...
				result.put("boolean_result", false);
				result.put(SysParaNameConst.STATUS_CODE, 2008);
				result.put(SysParaNameConst.RESULT_MSG, "详细地址输入错误");
				return result;
			}
		...
		if(obj.containsKey("topup_num")){
			...
			if (StringUtil.isEmptyByString(topup_num)
				...
				result.put("boolean_result", false);
				result.put(SysParaNameConst.STATUS_CODE, 2016);
				result.put(SysParaNameConst.RESULT_MSG, "充值积分数额输入错误");
				return result;
			}
		}
		if(obj.containsKey("consume_points_count")){
			...
			if (StringUtil.isEmptyByString(topup_num)
				...
				result.put("boolean_result", false);
				result.put(SysParaNameConst.STATUS_CODE, 2014);
				result.put(SysParaNameConst.RESULT_MSG, "消费积分数额输入错误");
				return result;
			}
		}
		// 明文用户
